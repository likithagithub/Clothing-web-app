...
function initializePage(product) {
    ...
    document.getElementById('price').textContent = product.price+ '.00';
...
function initializeThumbnails(thumbnailNames) {
    ...
    thumbnailNames.forEach(name => {
        ...
        imgElement.onclick = () => changeMainImage(`images/${name}`);
...
function calculateDiscountPercent(price, compareAtPrice) {
    ...
    const compareAtPriceNumber = parseFloat(compareAtPrice.replace(/[^0-9.-]+/g,""));
    console.log(`Parsed price: ${priceNumber}, Parsed compare at price: ${compareAtPriceNumber}`); 
    return Math.round(((compareAtPriceNumber - priceNumber) / compareAtPriceNumber) * 100);
